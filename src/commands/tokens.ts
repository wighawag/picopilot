  ...
  cli.command('tokens', {
    ...
    args: z.object({
      cart: z
        .string()
        .default('main.p8')
        .describe('The .p8 cart to count. Defaults to main.p8 in the current folder.'),
    }),
    ...
    output: z.object({
      ...
      pct: z
        .number()
        .describe('tokens as a percentage of the 8,192 budget (rounded); >100 means over budget.'),
      ...
      budget: z.number().describe('The token budget the percentage is against (8192).'),
      ...
      const cta = overBudget
        ? {
            ...
            commands: [
              {
                command: 'minify',
                description: 'Safe-minify the cart to reduce tokens without changing behaviour.',
              },
            ],
          }
        : undefined
