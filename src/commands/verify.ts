...
const defaultAdapterFactory: ShrinkoAdapterFactory = (env) => new ShellShrinkoAdapter({ env })
...
function checkIntegrity(text: string): IntegrityResult {
  ...
  try {
    ...
    if (e instanceof CartParseError) {
      return { ok: false, detail: `cart does not parse (${e.code}): ${e.message}` }
    }
    throw e
  }
  if (cart.serialize() !== text) {
    return {
      ok: false,
      detail: 'cart does not round-trip cleanly through the cart model (malformed sections)',
    }
  }
  return { ok: true }
}
  ...
  cli.command('verify', {
    ...
    args: z.object({
      cart: z
        .string()
        .default('main.p8')
        .describe('The .p8 cart to verify. Defaults to main.p8 in the current folder.'),
    }),
    ...
    // needs no PICO-8 env.
    env: z.object({
      PATH: z.string().optional().describe('Used to locate shrinko8 in the child process.'),
    }),
    output: z.object({
      status: z.literal('pass').describe('The overall gate result. Only a green gate reaches ok.'),
      scope: z
        .string()
        .describe(
      ...
      checks: z
        .object({
          integrity: z.boolean().describe('True when the cart parses and round-trips cleanly.'),
          tokens: z.boolean().describe('True when the cart is within the 8,192-token budget.'),
        })
    ...
    async run({ args, env, error, ok }) {
      const cartPath = isAbsolute(args.cart) ? args.cart : resolve(process.cwd(), args.cart)
      ...
      if (!integrity.ok || !tokensOk) {
        ...
        if (!tokensOk) {
          failed.push(`tokens: ${tokens} exceeds the ${TOKEN_BUDGET}-token budget`)
        }
        return error({
          code: 'verify-failed',
      ...
      return ok(
        ...
        {
          cta: {
            description: 'Static checks pass. Now confirm the cart actually boots:',
            commands: [
              {
                command: 'run',
                description:
