  ...
  new Promise((resolve) => {
    execFile(file, args, { env, encoding: 'utf8' }, (error, stdout, stderr) => {
      ...
      resolve({
        ...
        code: err !== null && typeof err.code === 'number' ? err.code : err === null ? 0 : null,
        stdout,
        stderr,
        spawnError: err !== null && typeof err.code === 'string' ? err : undefined,
      })
    })
  })
...
export class ShellShrinkoAdapter implements ShrinkoAdapter {
  ...
  async count(cartPath: string): Promise<ShrinkoResult<CountReport>> {
    ...
    return { ok: true, value: parseCount(`${outcome.stdout}\n${outcome.stderr}`) }
  }
