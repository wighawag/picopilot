...
export const PicopilotConfig = z.object({
  ...
  commands: z
    ...
    .optional()
    .describe('Per-command option defaults, keyed by command path (incur config layout).'),
})
...
export function readAllowMapOverlap(config: unknown): boolean {
  ...
  return parsed.data.commands?.gfx?.commands?.set?.options?.allowMapOverlap ?? false
}
