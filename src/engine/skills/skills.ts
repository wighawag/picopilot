...
export async function installSkills(options: InstallSkillsOptions): Promise<InstallSkillsResult> {
  ...
  if (commands === undefined) {
    throw new Error('installSkills: the given CLI has no command map (was it built by Cli.create?)')
  }
  ...
  return {
    paths: result.paths,
    agents: result.agents.map((a) => ({ agent: a.agent, path: a.path, mode: a.mode })),
