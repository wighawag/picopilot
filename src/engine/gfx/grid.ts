...
export function gridToNibbles(text: string): number[][] {
  ...
  if (rawLines.length !== GRID_SIZE) {
    throw new GfxGridError(`char grid must have ${GRID_SIZE} rows, got ${rawLines.length}`)
  }
  ...
  for (let r = 0; r < GRID_SIZE; r++) {
    ...
    if (line.length !== GRID_SIZE) {
      throw new GfxGridError(`char grid row ${r} must have ${GRID_SIZE} cells, got ${line.length}`)
    }
    const row: number[] = []
    for (let c = 0; c < GRID_SIZE; c++) row.push(charToNibble(line[c] as string, r, c))
    grid.push(row)
  }
  return grid
}
...
function assertNibbleGrid(grid: number[][]): void {
  if (grid.length !== GRID_SIZE) {
    throw new GfxGridError(`nibble grid must have ${GRID_SIZE} rows, got ${grid.length}`)
  }
  for (let r = 0; r < GRID_SIZE; r++) {
    const row = grid[r]
    if (row === undefined || row.length !== GRID_SIZE) {
      throw new GfxGridError(`nibble grid row ${r} must have ${GRID_SIZE} cells`)
    }
    for (let c = 0; c < GRID_SIZE; c++) {
      const v = row[c] ?? 0
      if (!Number.isInteger(v) || v < 0 || v > 15) {
        throw new GfxGridError(`nibble grid cell (${c}, ${r}) = ${v} out of range 0..15`)
      }
    }
  }
}
