import { GFX_HEIGHT, GFX_WIDTH, type GfxSheet, SPRITE_SIZE, spriteOrigin } from '../cart/index.js'
...
export function renderSheetPng(sheet: GfxSheet): Uint8Array {
  return encodePng(
    upscale(GFX_WIDTH, GFX_HEIGHT, (x, y) => sheet.getPixel(x, y), SHEET_RENDER_SCALE),
  )
}
...
export function upscale(
  ...
  if (!Number.isInteger(scale) || scale < 1) {
    throw new RangeError(`upscale factor must be a positive integer, got ${scale}`)
  }
