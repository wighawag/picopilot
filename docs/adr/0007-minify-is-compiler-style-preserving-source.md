...
- The load-bearing safety property is stronger than the spec asked for: picopilot has NO command that overwrites user-authored source. (`gfx set` writes the cart's binary `__gfx__`, not the Lua source; `minify` writes a new artifact.)
