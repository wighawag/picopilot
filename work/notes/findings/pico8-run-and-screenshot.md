...
- **`-x` IS genuinely headless (CORRECTED — an earlier note wrongly claimed it opens a window).** VERIFIED: `env -u DISPLAY pico8 -x cart.p8` runs and produces a screenshot with NO X display and no SDL/window errors in the log. The window-grab problem is specific to `-run`/`-h`/interactive modes, NOT `-x`. So the ONLY reason `-x` needs the external `timeout`/sentinel kill is that a CART CANNOT SELF-QUIT (see below) — that is independent of, and not, a windowing problem. `-x` + sentinel-kill is a clean headless automation path.
- **Honest limit to state in the skill:** screenshots require the CART to cooperate (self-screenshot via `extcmd("screen")`); there is no "screenshot an arbitrary running cart" hook. The agent edits the cart to add debug captures, then reverts them. (This is a cooperation requirement, NOT a headlessness limit — `-x` itself is fully headless.)
