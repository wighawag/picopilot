...
>
> ARCHITECTURE FORK to weigh FIRST (do NOT silently assume native): there are TWO viable run/test architectures (see the input finding's "second architecture" table). (a) NATIVE `pico8 -x` — lighter, uses only the user's PICO-8 binary; screenshots via `extcmd("screen")`, input via serial stdin `0x804`; but window-grab, needs timeout+sentinel. (b) WEB EXPORT — `EXPORT cart.html` run in a headless browser (Playwright); genuinely headless, browser-native canvas screenshots, rich GPIO 128-byte shared-array input (the documented community channel); costs an export step + a browser dep (+ `pico8.dat` for export). This task defaults to the NATIVE path (lightest, matches the run-and-screenshot finding), but the builder should CONSCIOUSLY confirm that vs. web (or note web as a follow-up) rather than assume it — the choice affects the whole `run` shape. If unsure it is a needs-answers question for a human.
>
