...
>
> ARCHITECTURE FORK to weigh FIRST (do NOT silently assume native): there are TWO viable run/test architectures (see the input finding's "second architecture" table). (a) NATIVE `pico8 -x` — lighter, uses only the user's PICO-8 binary; GENUINELY headless (verified: runs with `DISPLAY` unset, no window — the window-grab is `-run`/`-h` only, NOT `-x`); screenshots via `extcmd("screen")`, input via serial stdin `0x804`; needs a timeout+sentinel kill only because a cart can't self-quit (NOT a window problem). (b) WEB EXPORT — `EXPORT cart.html` run in a headless browser (Playwright); also headless, but its real edge is browser-native canvas screenshots (no cart-side `extcmd`) + a richer GPIO 128-byte shared-array input channel; costs an export step + a browser dep (+ `pico8.dat` for export). This task defaults to the NATIVE path (lightest, matches the run-and-screenshot finding), but the builder should CONSCIOUSLY confirm that vs. web (or note web as a follow-up) rather than assume it — the choice affects the whole `run` shape. If unsure it is a needs-answers question for a human.
>
...
## Decisions (recorded on completion)

Built on `main` (interactive session, no claim ceremony). All acceptance criteria met; full gate green (171 tests). In-scope choices made while building, surfaced for ratification:

- **Native path chosen (not web export).** Per ADR-0006's "native first" and the run-and-screenshot finding: `pico8 -x` + `-desktop <dir>` + stdout-sentinel kill. Web-export/browser is left as a non-foreclosed follow-up. The `Pico8Adapter` interface is the seam a web adapter would implement without touching the command.
- **Drift handled:** the task assumed `engine/pico8` already existed ("extend it"); it did NOT (only declared as a future seam in `engine/index.ts`). CREATED it, mirroring the `engine/shrinko` adapter+shell+injectable-runner shape.
- **Envelope shape:** `{ screenshots: string[], printh: string, exitReason: 'sentinel'|'timeout'|'exit', shotDir: string }`. `pico8Present` from the task's sketch was dropped as redundant: absence is already the structured `pico8-not-found` error, so a present run never needs a boolean.
- **`timeout` is a soft outcome, not a failure exit.** A backstop `timeout` still returns a report (exit 0) with a CTA to `verify`/`tokens`, rather than a nonzero exit. Rationale: the run DID produce screenshots/printh worth reading; the timeout is a signal to check for a fault, not a hard failure. (pico8-not-found and cart-not-found ARE nonzero.)
- **Sentinel match is an EXACT whole-line compare**, so an incidental substring in other output never trips it. Partial lines are buffered across stdout chunks.
- **Default backstop 15000ms**, `--backstop-ms` to override. **Default sentinel `__PICOPILOT_DONE__`**, `--sentinel` to override.
- **shotDir defaults to an isolated temp dir** (never `~/Desktop`); `--shot-dir` for a chosen path. The `-desktop` flag is the isolation lever; a test snapshots `~/Desktop` and asserts it is untouched.
- **Kill is process-GROUP (`detached` + negative pid SIGKILL)** so a hung PICO-8 and any child die together.
- **Input injection NOT built** (kept as the documented fast-follow); the envelope/flags stay open to a later `--input` arg.
- Verified LIVE against real PICO-8 (opt-in tier): a real cart screenshotted itself, printed the sentinel, `run` ended on `sentinel`, collected a 384x384 PNG + printh. CI covers the pico8-absent + sentinel/backstop/exit paths via an injected fake, no paid binary required.
