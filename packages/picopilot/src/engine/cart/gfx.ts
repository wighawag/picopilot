...
export class GfxSheet {
	...
	getSprite(n: number): number[][] {
		...
		for (let r = 0; r < SPRITE_SIZE; r++) {
			...
			for (let c = 0; c < SPRITE_SIZE; c++)
				row.push(this.getPixel(x0 + c, y0 + r));
			grid.push(row);
		}
		return grid;
	}
	...
	setSprite(n: number, grid: number[][]): void {
		if (grid.length !== SPRITE_SIZE) {
			throw new CartRangeError(
				`sprite grid must have ${SPRITE_SIZE} rows, got ${grid.length}`,
			);
		}
		const {x0, y0} = spriteOrigin(n);
		for (let r = 0; r < SPRITE_SIZE; r++) {
			...
			if (gridRow === undefined || gridRow.length !== SPRITE_SIZE) {
				throw new CartRangeError(
					`sprite grid row ${r} must have ${SPRITE_SIZE} cells`,
				);
			}
			for (let c = 0; c < SPRITE_SIZE; c++)
				this.setPixel(x0 + c, y0 + r, gridRow[c] ?? 0);
		}
	}
	...
	toBody(): string {
		...
		for (let y = 0; y < GFX_HEIGHT; y++) {
			let line = '';
			for (let x = 0; x < GFX_WIDTH; x++)
				line += (this.pixels[y]?.[x] ?? 0).toString(16);
			lines.push(line);
		}
	...
	private assertPixel(x: number, y: number): void {
		if (
			...
			throw new CartRangeError(
				`pixel (${x}, ${y}) out of range 0..${GFX_WIDTH - 1}`,
			);
		}
	}
}
 ...
 */
export function mapRowsForSprite(
	n: number,
