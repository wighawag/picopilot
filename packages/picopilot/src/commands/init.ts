...
export function registerInit(
	cli: Cli.Cli,
	installer: SkillsInstaller = installSkills,
): void {
	cli.command('init', {
		description:
		...
		args: z.object({
			dir: z
				...
				.default('.')
				.describe(
					'Target cart folder (created if missing). Defaults to the current directory.',
				),
		}),
		options: z.object({
			installSkills: z
		...
		output: z.object({
			dir: z
				.string()
				.describe('The absolute path of the scaffolded cart folder.'),
			files: z
				.array(z.string())
				.describe('The cart-relative names of the files written.'),
			tips: z
				.array(z.string())
				.describe(
					'One-line next-step tips (skills discovery + git init). NOT actions init took.',
				),
			installedSkills: z
				.array(z.string())
				.optional()
				.describe(
					'Skills installed by --install-skills (absent unless the flag was passed).',
				),
			wiredAgents: z
				.array(z.string())
				.optional()
				.describe(
					'Agents whose skill dirs --install-skills wired up (absent unless flagged).',
				),
		}),
		examples: [
		...
		async run({args, options, error}) {
			const targetDir = isAbsolute(args.dir)
				? args.dir
			...
			return {
				...
				tips: [
					...tips,
					`Skills installed: ${install.skills.join(', ')}. ${fallbackTip}`,
				],
