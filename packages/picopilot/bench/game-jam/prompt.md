...
The theme is deliberately generic. Interpret it on any layer(s) you like: graphics, mechanics, or story. Usually the ORIGINALITY comes from the MECHANICS, so favour a fresh mechanic over a fresh coat of paint. A clever one-idea game beats an ambitious unfinished one. ORIGINALITY is what separates a good jam entry from a forgettable one, and it is the axis most entries score WORST on, so spend your first minute of thinking here, not on code. AVOID the obvious first idea for the theme, and avoid the obvious COMBINATION too (fusing the two most cliche mechanics is still cliche). Some reliable ways to find a fresh idea WITHOUT extra scope, pick one:
- SUBVERT the expectation: make the theme mean the OPPOSITE of the obvious, or the twist be that the thing you assumed is inverted (you are not the hero, the goal is to LOSE, the button does something different each press).
- INVERT the ROLE: control the environment/obstacle/enemy/level instead of the avatar; help the thing you would normally fight; be the AI, the trap, or the physics.
- Make the CONSTRAINT the game: the limitation itself IS the mechanic (you cannot stop moving; you see only one tile; you share one health bar with the enemy).
- COMBINE two UNEXPECTED mechanics (not the two everyone pairs): a single simple rule doing double duty, where the pairing is surprising (the jump also rewinds time; dying spawns the next run's obstacle; the same input steers AND fires).
Whatever you pick, you should be able to say in one sentence WHY this is not the idea everyone else would make. If you cannot, pick a fresher angle. Keep the scope tiny regardless: one surprising idea, executed simply, beats an ambitious mush.
